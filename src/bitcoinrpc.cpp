...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop LendXpress server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "LendXpress server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: LendXpress-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("LendXpress-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use LendXpress";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n %s\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"LendXpress Alert\" admin@foo.com\n"),
                strWhatAmI.c_str(),
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("LendXpress-rpchand");
